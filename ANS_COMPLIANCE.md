...
John Hayes **forth2012-test-suite** (Block + Float `fp/` included): **0 errors** on all executed word sets (Block SAVE-INPUT/RESTORE/REFILL, EMPTY-BUFFERS, pictured-string `TCSIRIR2`/`TCSIRIR4`, REFILL spill, FP paranoia **END OF TEST** all pass). **Canonical run:** TZForth app → **`FROMLIB FLOAD HayesTest.fth`** (bundled `Resources/Library/HayesTest.fth` → `HayesTest/src/test.fth`). CLI `HAYES=1 swift /tmp/combined.swift` runs a smaller non-FP subset only. Results: **`TZForth/Library/HayesTest/src/HAYES-RESULTS.txt`**.
...
- Hayes reproduction: **`FROMLIB FLOAD HayesTest.fth`**; transcript baseline **`TZForth/Library/HayesTest/src/HAYES-RESULTS.txt`**.
