...
Automated tests (`FTEST=1`; see `TestTZForth.swift` header) cover load/comment harnesses plus **281** ANS spot-checks (Core, Core Ext, File-Access, String incl. `REPLACES`/`SUBSTITUTE`/`UNESCAPE`, Exception, Memory, Double, Locals, Programming-Tools, `REQUIRE`/`REQUIRED`, `DUMP`, `H.`, `>XID`, `>IN +!` skip, CATCH/THROW including nested CATCH, colon `CATCH`+`>R`, catchable mid-`FLOAD`, etc.). In-app: `ANS-VALIDATE` (same suite; overwrites `TZForth/ANS-VALIDATE.txt`, a tracked baseline in the Xcode project — regenerate anytime). Hayes **forth2012-test-suite** (no Block/Facility): **0 errors** — see `Tests/forth2012-test-suite/src/HAYES-RESULTS.txt`.
