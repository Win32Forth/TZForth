...
Automated tests (`FTEST=1`; see `TestTZForth.swift` header) cover load/comment harnesses plus **260** ANS Core/Core-Ext spot-checks (dictionary, file I/O, locals, double, memory, Programming-Tools, `REQUIRE`/`REQUIRED`, `DUMP`, `H.`, `>XID`, etc.). In-app: `ANS-VALIDATE`.
