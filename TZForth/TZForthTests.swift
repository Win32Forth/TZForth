...
""".write(to: fnOuterStop, atomically: true, encoding: String.Encoding.utf8)
        ...
        let maxGrowMB = 64
        let currentMB = max(1, self.memory.count / (1024 * 1024))
        ...
        resetTest()
        self.feedLine(": tzbslash1 9 ; \\ noop")
        ansTest("\\ compile comment", "tzbslash1 .", "9")
        resetTest()
        collected = ""
        self.feedLine(": TZWRDEF1 1 ;")
        self.feedLine(": TZWRDEF1 2 ;")
        ansTotal += 1
        if collected.contains("TZWRDEF1 isn't unique") {
            ansPassed += 1
            results += "TEST6 WARNING redef: pass\n"
        } else {
            results += "TEST6 WARNING redef: FAIL out='\(collected.trimmingCharacters(in: .whitespacesAndNewlines))'\n"
        }
        collected = ""
        self.feedLine("WARNING OFF")
        self.feedLine(": TZWRDEF1 3 ;")
        ansTotal += 1
        if !collected.contains("isn't unique") {
            ansPassed += 1
            results += "TEST6 WARNING off: pass\n"
        } else {
            results += "TEST6 WARNING off: FAIL out='\(collected.trimmingCharacters(in: .whitespacesAndNewlines))'\n"
        }
        self.feedLine("WARNING ON")
        resetTest()
        _ = fm.changeCurrentDirectoryPath(tmp.path)
        self.logicalCurrentDirectory = tmp.path
        self.feedLine("FILE-ECHO OFF")
        ...
        results += "=== TZForth Extended-Character (ANS 18.6 UTF-8 memory + string words) ===\n"
        ...
        resetTest()
        self.feedLine("DECIMAL")

        results += "=== TZForth Extended-Character (ANS 18.6.2 pictured — XHOLD) ===\n"
        ...
        results += "=== TZForth Extended-Character (ANS 18.6.2 display width — XC-WIDTH, X-WIDTH) ===\n"
        ...
        results += "=== TZForth Extended-Character (ANS 18.3.2 ENVIRONMENT?) ===\n"
        ...
        results += "=== TZForth Float Tier A (ANS 12 — minimal IEEE 64-bit, separate F stack) ===\n"
        ...
        results += "=== TZForth Float Tier C (REPRESENT / FS. / FE. / F.) ===\n"
