...
\
\ Reproduce validated results (see HAYES-RESULTS.txt in this folder):
\   1. Build and run the TZForth app.
\   2. CHDIR to this directory, or use bare FLOAD once and pick any file here
\      (sandbox bookmark). Logical cwd must be Tests/forth2012-test-suite/src/.
\   3. FLOAD test   (or FLOAD test.fth)
\
\ Pass criteria:
